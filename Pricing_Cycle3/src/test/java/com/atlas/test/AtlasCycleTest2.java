...
public class AtlasCycleTest2 {
	...
	public void test() {
		...
		CustomThreadPoolExecutor executor = new CustomThreadPoolExecutor(10, 10, 5000, TimeUnit.MILLISECONDS,
				blockingQueue);
		...
		executor.setRejectedExecutionHandler(new RejectedExecutionHandler() {
			...
			public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
				...
				System.out.println("Lets add another time : " + r.getClass());
				executor.execute(r);
			}
		});
		...
		while (true) {
			...
			System.out.println(cycle.getBreaks().getBreakType());
			 executor.execute(cycle);
		//	new AtlasCycle().getCyclePrice(cycle);
			if (threadCounter == 100) {
			System.out.println("\ntotal cycle count :: " + threadCounter);
			System.out.println("total cycle Price :: " +cycle.getTotalPrice());
			break;
			}
		}
...
//		Double cyclePrice = new AtlasCycle().getCyclePrice(cycle);
			AtlasCycle atlasCycle = new AtlasCycle() {
			{
				setCycle(cycle);
			}
			};
